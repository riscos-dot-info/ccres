...
static int copy_quoted_string(DATA *data, char *outP, const char *inP, int maxSize)
{
  ...
  if (c >= ' ' && c != '"')
    ...
    {
      if (c != '"')
        ccres_report(data, report_error, getlinenr(data, inP), "Incorrect quoted string '%s'", fixInP);
    }
  if (maxSize >= 1)
    *outP++ = '\0';
  return outP - fixOutP;
...
static bool put_string(DATA *data, const char *pszIn, int nOffset, char *object, const STRINGLIST *StringList)
{
  ...
  if (StringList->nTable == iol_STRING)
    ...
    {
      ccres_report(data, report_error, getlinenr(data, pszIn), "Unexpected string table type (%d)", StringList->nTable);
      return false;
    }
      ...
      if (pszLimit[0] == '*')
      ...
      else
        {
          ...
          if (cbLimit == 0 && cbEntry <= 1)
            ...
            {
              ccres_report(data, report_warning, getlinenr(data, pszIn), "increased size value for %s from %d to %d to fully include string <%.*s>", StringList->pszEntry, cbLimit, cbEntry, cbEntry ? cbEntry - 1 : cbEntry, &pTable->pstr[pTable->ref]);
              cbLimit = cbEntry;
            }
        }
    }
  ...
  if (!add_to_reloc_table(&data->RelocTable, StringList->nEntry + nOffset, nTable))
    ccres_report(data, report_error, getlinenr(data, pszIn), "Failed to add relocation because of lack of free memory");
...
// ref == -1 is valid Toolbox reference meaning the empty string.
  if (pszTable == NULL || ref == -1)
    ...
    {
      ccres_report(data, report_error, 0, "string index %d is out-of-bounds (string table size is %d)", ref, tableSize);
      *outSizeP = 0;
      return "";
    }
...
static void get_string(DATA *data, FILE *hf, const TOOLBOXSMTABLE *strMsgTableP, const char *objectP, const STRINGLIST *StringList, const char *pszIndent)
{
  ...
  switch (StringList->nTable)
    ...
    case iol_MSG:
      ...
      break;
    default:
      ccres_report(data, report_error, 0, "Unexpected string table type (%d)", StringList->nTable);
      return;
    }
  ...
  if (StringList->pszLimit != NULL)
    {
      ...
      if ((cbLimit == 0 && cbLimit < strSize)
        ...
        ccres_report(data, report_warning, 0, "string <%.*s> is longer than its given limit value %d", strSize, pstr, cbLimit);
      ...
      if (n == nFlags)
        {
          if (pstrFlags[0] == '0' && pstrFlags[1] == 'x')
            ...
            ccres_report(data, report_error, getlinenr(data, pstrFlags), "Unknown flag '%.*s'", cb, pstrFlags);
        }
  ...
  if (*pstrFlags > ' ')
    {
      ...
      ccres_report(data, report_error, getlinenr(data, pstrFlags), "Unknown variable '%s'", pstrFlags);
    }
          ...
          if ((pszEntry = parse(data, pszIn, ObjectList->pszEntry)) != NULL)
            {
              switch (ObjectList->nTable)
                ...
                default:
                  ccres_report(data, report_error, getlinenr(data, pszIn), "Unknown iol_ value (%d)", ObjectList->nTable);
                  break;
                }
            }
          break;
      ...
      if (ObjectList->nTable != iol_OBJECT)
        { // do nothing for res2text
          ...
          switch (ObjectList->nTable)
            ...
            default:
              ccres_report(data, report_error, 0, "Unknown iol_ value (%d)", ObjectList->nTable);
              break;
            }
        }
    }
          ...
          while (*p++ >= ' ')
            {
              if (p >= pszEnd)
                {
                  ccres_report(data, report_error, getlinenr(data, pszIn), "Missing brace }");
                  return NULL;
                }
            }
        }
  ...
  if (object->string_table_offset != -1)
    {
      if (object->string_table_offset != nextMinOffset)
        {
          ccres_report(data, report_error, 0, "Unexpected string table offset (is 0x%x, expected 0x%x)", object->string_table_offset, nextMinOffset);
          return;
        }
    }
  if (object->message_table_offset != -1)
    {
      if (object->message_table_offset < nextMinOffset)
        {
          ccres_report(data, report_error, 0, "Unexpected message table offset (is 0x%x, expected to be bigger or equal than 0x%x)", object->message_table_offset, nextMinOffset);
          return;
        }
  ...
  if (object->relocation_table_offset != -1)
    {
      if (object->relocation_table_offset < nextMinOffset)
        {
          ccres_report(data, report_error, 0, "Unexpected relocation table offset (is 0x%x, expected to be bigger or equal than 0x%x)", object->relocation_table_offset, nextMinOffset);
          return;
        }
      if ((unsigned int)object->relocation_table_offset != offsetof(toolbox_relocatable_object_base, rf_obj)
          + object->rf_obj.size)
        {
          ccres_report(data, report_error, 0, "Unexpected relocation table offset (is 0x%x, expected to be 0x%x)", object->relocation_table_offset, offsetof(toolbox_relocatable_object_base, rf_obj) + object->rf_obj.size);
          return;
        }
    }
